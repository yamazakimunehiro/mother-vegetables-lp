    ...
    <section id="product" className="py-20 bg-gradient-to-b from-gray-50 to-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <p className="text-sm text-gray-600 mb-4 tracking-wider">商品ラインナップ</p>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-20">
            {products.map((product) => (
              <div
                key={product.size}
                className={`size-selector bg-white rounded-2xl p-8 border-2 text-center shadow-lg hover:shadow-xl transition-all duration-300 relative overflow-hidden group cursor-pointer ${
                  product.popular ? 'border-red-500' : 'border-gray-200'
                } ${selectedSize === product.size ? 'ring-4 ring-gray-800 ring-opacity-20' : ''}`}
                onClick={() => setSelectedSize(product.size)}
              >
                {product.popular && (
                  <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-red-500 text-white px-6 py-1 rounded-full text-sm font-bold">
                    人気No.1
                  </div>
                )}
                
                <div className="absolute inset-0 opacity-5">
                  <div className="absolute inset-0" style={{backgroundImage: 'radial-gradient(circle at 1px 1px, #333 0.5px, transparent 1px)', backgroundSize: '20px 20px'}} />
                </div>
                ...
                <div className="relative z-10">
                  <div className="product-image-container aspect-square rounded-2xl mb-6 overflow-hidden relative">
                    <div className="absolute inset-0 bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
                      <div className="text-center">
                        <p className="text-4xl font-light text-gray-600 mb-2">{product.size}</p>
                        <p className="text-sm text-gray-500">Product Photo</p>
                      </div>
                    </div>
                  </div>
                  
                  <h3 className="text-2xl font-light mb-2 text-gray-900">{product.title}</h3>
                  ...
                  <div className="mb-4">
                    <p className="text-sm text-gray-500 line-through mb-1">通常価格 {product.originalPrice}</p>
                    <p className={`text-3xl font-light mb-1 ${product.popular ? 'text-red-600' : 'text-gray-900'}`}>
                      {product.price}
                    </p>
                  ...
                  <button className={`w-full px-6 py-3 transition-all duration-300 relative overflow-hidden group/btn ${
                    ...
                    <div className="absolute inset-0 bg-gradient-to-r from-gray-700 to-gray-800 transform translate-x-full group-hover/btn:translate-x-0 transition-transform duration-300" />
                  </button>
                </div>
              </div>
            ))}
          ...
          {/* Ingredients Section */}
          <div className="mt-20">
            <h3 className="text-2xl font-light mb-12 text-center text-gray-900 tracking-wide">全成分・効果</h3>
            <div className="grid md:grid-cols-2 gap-8">
              <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-8 border border-gray-200 shadow-md hover:shadow-lg transition-all duration-300">
                <div className="flex items-center mb-6">
                  ...
                  <h4 className="text-xl font-light text-gray-800">主要成分</h4>
                </div>
                <div className="space-y-4">
                  {ingredients.map((ingredient, index) => (
                    <div key={index} className="border-l-2 border-gray-300 pl-4 hover:border-gray-500 transition-colors duration-300">
              ...
              <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-8 border border-gray-200 shadow-md hover:shadow-lg transition-all duration-300">
                <div className="flex items-center mb-6">
                  ...
                  <h4 className="text-xl font-light text-gray-800">期待できる効果</h4>
                </div>
                <div className="space-y-4">
                  {effects.map((effect, index) => (
                    <div key={index} className="flex items-start">
                      <span className="inline-block w-6 h-6 bg-gray-200 rounded-full flex-shrink-0 mt-0.5 mr-3">
                        <svg viewBox="0 0 24 24" className="w-6 h-6 text-gray-600">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700">{effect.title}</p>
